...
\begin{table}[ht!]
	...
	\hline
	\texttt{trade\_data} & 265.53 & 56.92 & 45.12 & 0.34 & 0.10 \\
	\hline
	\texttt{market\_orders} & 684.14 & 200.70 & 182.68 & 0.11 & 0.05 \\
	\hline
	\texttt{tweets} & 3997.58 & 2182.62 & 1439.69 & 0.10 & 0.05 \\
	\hline
	\end{tabularx}
\end{table}
...
\begin{enumerate}
	...
	\item \texttt{ORC} значительно быстрее читается~---~время чтения \texttt{ORC} файлов в $\sim$3~--~4 раза меньше по сравнению с \texttt{Parquet} (0.05~--~0.10~с против 0.10~--~0.34~с);
	\item оба формата значительно превосходят csv~---~размер файлов уменьшается в $\sim$1.8~--~5.9 раз, а скорость чтения увеличивается в $\sim$10~--~50 раз (время чтения csv: 1.84~--~5.05~с против 0.05~--~0.34~с для сжатых форматов).
\end{enumerate}
